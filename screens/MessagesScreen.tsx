...
const MessagesScreen: React.FC = () => {
    ...
    return (
        <div className="w-full h-full flex flex-col text-white">
            {/* Header: keep bg as before, back button matches AuthScreen, initial circle stroke white */}
            <header className="p-4 flex items-center border-b border-white/10 bg-[#2E2E55]">
                <button
                    onClick={() => setScreen(Screen.DASHBOARD)}
                    className="mr-4 text-pink-300 flex items-center gap-2"
                    aria-label="Back"
                >
                    <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="19" y1="12" x2="5" y2="12"></line><polyline points="12 19 5 12 12 5"></polyline></svg>
                </button>

                <div className="flex items-center">
                    <div className="w-10 h-10 rounded-full bg-white/10 mr-3 flex items-center justify-center border" style={{ borderColor: '#fff', borderWidth: 2 }}>
            ...
            {/* Middle section: keep original bg (no bg override) */}
            ...
            {/* Footer: recolor bar to match header, input cylinder pure white */}
            <footer className="p-4 border-t border-white/10 bg-[#2E2E55]">
                <form onSubmit={handleSend} className="flex items-center">
                    <div className="flex-grow bg-white rounded-full flex items-center px-2 py-1 shadow-md">
                        <input
                            type="text"
                            value={newMessage}
                            onChange={(e) => setNewMessage(e.target.value)}
                            placeholder="Type a message..."
                            className="flex-grow bg-transparent text-[#232347] px-4 py-2 focus:outline-none placeholder-[#A0A0C0]"
                        />
                    </div>
                    <button
                        type="submit"
                        className="ml-3 bg-[#FFA8A8] rounded-full p-3 text-[#3A3A69] hover:bg-pink-300 transition-colors shadow-md"
                        aria-label="Send"
                    >
